...
class RequestProgram {
    static async handle(name) {
        ...
        if (checkFolder) {
            ...
            if (doesFileExist == false) {
                ...
                return baseCommand_1.default.error(name + "_request.ts already exist. Modify request validator name and try again");
            }
        }
    }
    static async nextStep(name) {
        ...
        fs_1.default.appendFile("./App/Http/Requests/" + validationName + ".ts", this.generateRequest(validationName), function (err) {
            if (err)
                return baseCommand_1.default.error(err.errno);
            baseCommand_1.default.success(validationName + "_validation.ts class successfully generated in App/Http/Requests folder");
            return true;
        });
    }
