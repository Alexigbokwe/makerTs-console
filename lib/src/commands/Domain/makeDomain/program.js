...
const program_2 = require("../../route/program");
const program_3 = require("../../sqlModel/program");
const program_4 = require("../../nosqlModel/program");
const program_5 = require("../../makeService/program");
...
class MakeDomainProgram {
    ...
    static async routeFolder(path, name) {
        ...
        fs_1.default.appendFile(`${path}/Routes/index.ts`, await program_2.RouteProgram.routeBody(name), function (err) {
            if (err)
                throw err;
    ...
    static modelBody(name, orm) {
        if (orm !== CommandTypes_1.ORM.Mongoose) {
            return program_3.SqlProgram.modelBody(name, orm);
        }
        else {
            return program_4.NoSqlProgram.generateModel(name);
        }
    }
    ...
    static loadAbstractService(servicePath, name) {
        fs_1.default.appendFile(`${servicePath}/${name}Service.ts`, program_5.ServiceProgram.generateServiceAbstractClass(name), function (err) {
            if (err)
                throw err;
    ...
    static loadService(servicePath, name) {
        fs_1.default.appendFile(`${servicePath}/${name}ServiceImpl.ts`, program_5.ServiceProgram.generateService(name, false), function (err) {
            if (err)
                throw err;
