...
class MakeDomainProgram {
    ...
    static async modelFolder(path, name) {
        ...
        fs_1.default.appendFile(`${path}/Model/${name}Model.ts`, this.modelBody(name), function (err) {
            if (err)
                throw err;
    ...
    static loadService(servicePath, name) {
        fs_1.default.appendFile(`${servicePath}/index.ts`, program_5.default.generateService(`${name}Service`), function (err) {
            if (err)
                throw err;
