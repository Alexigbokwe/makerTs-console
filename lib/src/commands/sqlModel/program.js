...
class SqlProgram {
    ...
    static async createModel(modelName, resource) {
        if (resource == "Generation migration with sql model") {
            ...
            fs_1.default.appendFile("./App/Model/" + modelName + "Model.ts", await this.modelBodyWithMigration(modelName), function (err) {
                if (err)
            ...
            spinner.color = "magenta";
            spinner.text = "Generating Model";
            fs_1.default.appendFile("./App/Model/" + modelName + "Model.ts", this.modelBody(modelName), function (err) {
                if (err)
                    baseCommand_1.default.error(err);
                baseCommand_1.default.success("\n" + modelName + "Model.ts class successfully generated in App/Model folder");
                spinner.color = "green";
    ...
    //   return body;
    // }
    static ObjecionModelBody(modelName, tableName) {
        let body = `"use strict";
