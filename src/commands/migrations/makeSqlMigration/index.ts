...
class MakeSqlMigrationCommand {
  static async handle(program: Program) {
    ...
    const commandArguments: CommandArgument[] = [{ name: "migrationName", mode: "REQUIRED", description: "The name of the migration file" }];
    ...
    const cmd = program.command(`make-sql-migration ${commandSignature}`).alias("msqlm").description("Create a new migration file");
